...
then
	...
	then
        ...
        echo "[-] Run ShapeOS"
        docker run -it -e USER=${USER} -v /opt/shapeos:/custom  -v /opt/user_data:/home/ -v /var/run/docker.sock:/docker.sock -v /dev/:/dev/ --privileged --ipc host --net host bres/shape-os	
else
	echo Welcome
fi
